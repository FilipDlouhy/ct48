...
function page() {
    ...
    <div>
        <div className='w-full h-11 mt-8 flex items-center justify-between '>
             <button className='my-6 w-52 h-8 text-white bg-red-400 text-lg font-semibold    rounded-lg hover:w-64 duration-300 cursor-pointer' > Go to your Blogs</button>

             <button className='my-6 w-52 h-8 text-white bg-red-400 text-lg font-semibold    rounded-lg hover:w-64 duration-300 cursor-pointer' > Update Blog</button>
        </div>
        ...
        <div className='BlogPageReporter flex flex-col items-center p-5'>
            <input className=" bg-red-400 text-center text-white font-semibold mb-4 cursor-pointer"  onChange={(e)=>{
                if(e.target.files){
                    setImg(URL.createObjectURL(e.target.files[0])   )
                }
                console.log(img)
            }} type="file"></input>    
             {img ?<img  src={img.toString()}  className='h-2/5 w-11/12'></img> : <img className='h-2/5 w-11/12'></img>}
